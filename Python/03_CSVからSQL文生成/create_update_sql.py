...
if __name__ == "__main__":
    try:
        ...
        for item in import_selections:
            ...
            update_sql_list = _csv.get_update_sql(export_csv_file,
                                                  item["db_table"],
                                                  item["key_infos"],
                                                  encoding)

            if len(update_sql_list) < 1:
                raise Exception(
                    "Error: CSVから" + item["db_table"] + "のUpdate用Sql文作成が失敗しました"
                )
            else:
                print("Success: CSVから" + item["db_table"] 
                      + "のUpdate用Sql文作成が完了しました")

            #  Sql文の結果を出力
            export_sql_file = _fileio.combine_file_path(
                import_dir, item["export_file"] + ".sql"
            )
            #  Update用SQLをファイルに出力する
            result = _fileio.write_list_tofile(update_sql_list, export_sql_file)

            if not result:
                raise Exception(
                    "Error: Update用" + item["export_file"] 
                    + ".sql文のファイル出力が失敗しました"
                )
            else:
                print("Success: Update用" 
